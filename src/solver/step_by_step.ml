module Make(Abs : Domain_signature.AbstractCP) (Dr:Drawer_sig.Drawer with type t = Abs.t) = struct
  ...
  let explore (abs:Abs.t) (constrs:Csp.constrs) v1 v2 =
    ...
    let switch abs =
      ...
      select abs
    in
    let rec aux abs cstrs iter =
      switch abs;
      Unix.pause();
      match consistency abs cstrs with
      | Empty -> draw_n abs; selected := None;
      | Full abs' -> draw_n abs; selected := None; draw_s abs'
      ...
      | Maybe(abs',cstrs,_) ->
         ...
         if !Constant.pruning then
           ...
           List.iter (fun elem ->
               aux elem cstrs (iter +1)
	           ) splits
    in aux abs constrs 0
